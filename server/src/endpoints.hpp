...
static std::unordered_map<int, std::string> todos;
static int lastId = 0;
...
returnType AddTodo(CppHttp::Net::Request req) {
    ...
    todos[lastId + 1] = body["todo"].get<std::string>();

    std::string html = "<div id='e" + std::to_string(lastId+1) + "' class='flex flex-row bg-blue-500 min-h-[10vh] w-[35vw] rounded-[10pt] items-center text-wrap overflow-hidden'><div class='flex flex-row items-center relative w-[100%] h-[100%]'><p class='pl-[10px] py-[12px] text-white text-wrap whitespace-normal overflow-auto break-all'>" + todo + "</p><button class='absolute right-[0px] ml-[10px] mr-[10px]' hx-target='#e" + std::to_string(lastId + 1) + "' hx-swap='outerHTML' hx-delete='/remove' hx-vals='{\"todo\": " + std::to_string(lastId + 1) + "}' hx-ext='json-enc'><svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' id='checkbox' class='w-[4vh] h-[4vh]'><path d='M405.333 106.667v298.666H106.667V106.667h298.666m0-42.667H106.667C83.198 64 64 83.198 64 106.667v298.666C64 428.802 83.198 448 106.667 448h298.666C428.802 448 448 428.802 448 405.333V106.667C448 83.198 428.802 64 405.333 64z'></path></svg></button></div></div>";
...
returnType RemoveTodo(CppHttp::Net::Request req) {
    ...
    if (todo < 0 || todos.find(todo) == todos.end()) {
...
returnType GetTodos(CppHttp::Net::Request req) {
    ...
    for (auto [id, todo] : todos) {
        todosHtml += "<div id='" + std::to_string(id) + "' class='flex flex-row bg-blue-500 min-h-[10vh] w-[35vw] rounded-[10pt] items-center text-wrap overflow-hidden'><div class='flex flex-row items-center relative w-[100%] h-[100%]'><p class='pl-[10px] py-[12px] text-white text-wrap whitespace-normal overflow-auto break-all'>" + todo + "</p><button class='absolute right-[0px] ml-[10px] mr-[10px]' onClick='removeTodo(this)'><svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' id='checkbox' class='w-[4vh] h-[4vh]'><path d='M405.333 106.667v298.666H106.667V106.667h298.666m0-42.667H106.667C83.198 64 64 83.198 64 106.667v298.666C64 428.802 83.198 448 106.667 448h298.666C428.802 448 448 428.802 448 405.333V106.667C448 83.198 428.802 64 405.333 64z'></path></svg></button></div></div>";
    }
